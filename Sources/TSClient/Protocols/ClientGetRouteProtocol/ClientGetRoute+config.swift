...
public extension OutputableRoute where OutputData : Sendable , Self : EncoderDecoder {
    ...
    static func mockClient(always value : OutputData) -> MockGetServer<OutputData> {
       MockGetServer(for: Self.self, always: value)
    }
    
    ///Mock config that will always throw with your provided error
    static func mockClient(throws value : Self.Failure) ->MockGetServer<Self.Failure> {
        MockGetServer(for: Self.self, throws: value)
